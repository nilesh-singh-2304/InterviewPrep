...
const History = () => {
  ...
  return (
    <div>
      <div className='text-xl font-bold'>Your Feedbacks</div>
      { roomList?.length==0 && <div className='text-gray-500'>You dont have any prev Feedbacks</div>}

      <div>
        {roomList?.map((item,index)=>  ( item.category ==='Mock Interview' || item.category ==='Question-Answer Preparation') && (
            <Link href={`/view-summary/${item._id}`} key={index} className='cursor-pointer hover:scale-105 transition-all duration-200 ease-in-out'>
